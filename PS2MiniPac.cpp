/*
  PS2MiniPac.cpp - PS2Keyboard library - MiniPac customised ediition
  Copyright (c) 2007 Free Software Foundation.  All right reserved.
  Written by Christian Weichel <info@32leaves.net>
  
   ** streamlined for MiniPac implementation by Jon Payne (jon@jonnypayne.com) 2015 -
   Please apply the MiniPac setup to the minipac first, this avoids special characters and up and down arrows.
   MiniPac disables autorepeat for all keys except up and down arrows, in PS/2 mode.
  ...
  ** Modified for easy interrup pin assignement on method begin(datapin,irq_pin). Cuningan <cuninganreset@gmail.com> **
  ...
  or http://www.pjrc.com/teensy/td_libs_PS2Keyboard.html
	...
	built on version 2.4 of PS2Keyboard.cpp as per history below
...
// output.  The miniPAC only has 32 outputs but I will leave the full US layout in here in case someone wishes to adapt for a larger PAC product.
// With the included minipac config file, the lowest char that should be returned is 48 and the highest is 122.

const PROGMEM PS2Keymap_t PS2Keymap_MiniPac = {
  // without shift
	...
	while (1) {
		...
		if (s == 0xF0) {
			...
			if (state & BREAK) {
				...
				//continue; //dont do this - we want the keyup char as well.
			}
			if (s == 0x12) {
				state |= SHIFT_L;
				continue;
...
bool PS2MiniPac::keyup() {
	return keyupresult;
}

PS2MiniPac::PS2MiniPac() {
  // nothing to do here, begin() does it all
}

void PS2MiniPac::begin(uint8_t data_pin, uint8_t irq_pin, const PS2Keymap_t &map) {
  uint8_t irq_num=255;
